...
const API_URL = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/fetch-world-gold-price`;
...
const WorldGoldPrice = () => {
  const [data, setData] = useState<WorldGoldData | null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (mounted) setError('Không thể tải giá vàng thế giới');
      } finally {
        ...
        <div className="glass-card overflow-hidden">
          {loading ? (
            <div className="flex items-center justify-center py-12">
              ...
              <span className="ml-2 text-sm text-muted-foreground font-body">Đang tải giá vàng thế giới...</span>
            </div>
          ) : error && !data ? (
            ...
            <>
              ...
              <div className="px-4 md:px-6 py-6 flex flex-col md:flex-row items-center justify-between gap-4 border-b border-border/50">
                <div className="text-center md:text-left">
                  <p className="text-sm text-muted-foreground font-body mb-1">Vàng giao ngay (XAU/USD)</p>
